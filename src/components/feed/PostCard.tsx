    ...
    <article className="bg-[#13131d] border border-[#1e1e2e] rounded-xl p-4 hover:border-[#2e2e4a] transition-colors">
      <div className="flex items-start gap-3 mb-3">
        <Link
          ...
          className="w-9 h-9 rounded-full bg-[#1a1628] flex items-center justify-center text-xs font-medium text-[#9f8fdd] hover:bg-[#241d3a] transition-colors shrink-0"
        >
          {initials}
        </Link>
      ...
      <Link href={`/posts/${post.id}`} className="block group">
        <h2 className="text-sm font-medium text-neutral-100 group-hover:text-[#e8c84a] transition-colors mb-1.5">
          {post.title}
        </h2>
      ...
      <div className="flex items-center gap-4 mt-4">
        <button
          ...
          className={`flex items-center gap-1.5 text-xs transition-colors ${
            liked ? 'text-[#e8c84a]' : 'text-neutral-600 hover:text-[#e8c84a]'
          }`}
        >
