...
export default async function ProfilePage({ params }: { params: { username: string } }) {
  ...
  const userRes = await db.execute(
    `SELECT id, username, psn_id, bio, avatar_url, created_at FROM users WHERE username = '${targetUsername}'`
  )
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <div className="space-y-3">
        <h2 className="text-sm font-medium text-neutral-500">Posts de {user.username as string}</h2>
